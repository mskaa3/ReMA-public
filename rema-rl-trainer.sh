...
COMMAND="unset ROCR_VISIBLE_DEVICES;python3 -m pip install --force-reinstall math-verify;python3 -m pip install --force-reinstall --no-deps antlr4-python3-runtime==4.9.3;export PYTHONPATH=/root/ReMA-public/src:/verl:\$PYTHONPATH;python3 -m verl.rema_separated_trainer.main_ppo --config-path=/home/ajanz/projects/ReMA-public/config --config-name=rema-rl.yaml algorithm.hierarchy.enable=True +algorithm.switch_agent.enable=True +algorithm.switch_agent.level=step +algorithm.switch_agent.freq=1 +algorithm.switch_agent.agent_roles=[decomposer,selector,algebra_worker,functional_analysis_worker,general_math_worker,finalizer] +algorithm.switch_agent.start_agent=decomposer +algorithm.switch_agent.model_paths=[${MODEL_PATH},${MODEL_PATH}]"

...
def render_decomposer_prompt(
    ...
    if soft_max_hops_hint is not None:
        node_budget_lines.append(
            f"- Prefer at most {int(soft_max_hops_hint)} nodes when possible; compact decompositions are usually better."
        )
    ...
    return (
        "OUTPUT CONTRACT:\n"
        "- You may optionally think inside <decomposer_scratchpad>...</decomposer_scratchpad>.\n"
        "- You must return exactly one <decomposition_plan> block.\n"
        "- Use only these keys inside <decomposition_plan>: TARGET_QUANTITY, FINAL_ANSWER_FORMAT_HINT, FINAL_NODE_ID, NODE_ID, INSTRUCTION, DEPENDENCIES, REQUIRED_SKILLS, REQUIRED_SKILLS_NOTE.\n"
        "- Use this skeleton:\n"
        "<decomposer_scratchpad>\n"
        "Node 1 should rewrite the equation, then node 2 can isolate the value asked for.\n"
        "</decomposer_scratchpad>\n"
        "<decomposition_plan>\n"
        ...
        "- Prefer a compact but meaningful decomposition. A good default is about 3 to 5 nodes when the task has real multi-step structure.\n"
        "- Prefer a few smaller meaningful steps over a single node; one-node and shallow two-node decompositions are usually only appropriate when the task is genuinely atomic or cannot be usefully divided.\n"
        "- Every non-final node must produce a concrete intermediate mathematical artifact that is actually needed by a downstream node.\n"
        "- Keep node instructions short and concrete.\n"
        ...
        "- Do not add wrapper, restatement, or bookkeeping nodes whose only job is to say `return the final answer`, `provide the final answer`, `state the answer`, `assert the answer`, `express the answer`, or similarly generic text.\n"
        "- Do not create a final node that only repeats, reformats, or restates a result that an earlier node already computed in the required form.\n"
        "- If one node already computes TARGET_QUANTITY in the required FINAL_ANSWER_FORMAT_HINT, that node must be the FINAL_NODE_ID instead of adding another node.\n"
        "- The final node must perform a real remaining mathematical step from its dependencies; if no real step remains, do not add another node.\n"
        ...
        "- Forbidden output patterns: markdown fences, JSON, bullets, prose outside tags, or keys other than the allowed ones.\n\n"
        f"TASK_ID: {task.task_id}\n"
        f"TASK: {task.prompt}\n"
        "Return ONLY the optional <decomposer_scratchpad> block followed by the required <decomposition_plan> block."
    )
...
def render_worker_prompt(
    ...
    return (
        ...
        "- Format example (replace the content with the actual task-specific reasoning and result; do not copy these literals):\n"
        "<worker_scratchpad>\n"
        "From 2x = 8, divide both sides by 2.\n"
        "</worker_scratchpad>\n"
        "<worker_result>\n"

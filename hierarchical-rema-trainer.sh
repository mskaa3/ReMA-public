...
#SBATCH --verbose
#SBATCH --output=/lustre/tmp/slurm-%j.out
#SBATCH --error=/lustre/tmp/slurm-%j.err
...
DEFAULT_RUN_TMP_BASE=${TMPDIR:-/tmp/${USER:-user}}
if [[ "${SLURM_NNODES:-1}" -gt 1 && -n "${TMPDIR_LUSTRE:-}" ]]; then
    DEFAULT_RUN_TMP_BASE="$TMPDIR_LUSTRE"
fi
RUN_ROOT=${RUN_ROOT:-$DEFAULT_RUN_TMP_BASE/hierarchical_rema_${RUN_KIND}_${JOB_ID}}
TASK_SOURCE_ARG=${1:-}
...
DEFAULT_PERSIST_LOCAL_DIR=${SLURM_SUBMIT_DIR:-$PWD}/outputs/${OUTPUT_SUBDIR}/${JOB_ID}
PERSIST_LOCAL_DIR=${PERSIST_LOCAL_DIR:-$DEFAULT_PERSIST_LOCAL_DIR}
if [[ "$RUN_KIND" == "train" && -n "${TMPDIR_LUSTRE:-}" ]]; then
    DEFAULT_DISTRIBUTED_OFFLINE_INPUT_ROOT="${TMPDIR_LUSTRE}/hierarchical_rema_train_${JOB_ID}/_distributed_offline_grpo"
else
    DEFAULT_DISTRIBUTED_OFFLINE_INPUT_ROOT="${DEFAULT_PERSIST_LOCAL_DIR}/_distributed_offline_grpo"
fi
DISTRIBUTED_OFFLINE_INPUT_ROOT=${DISTRIBUTED_OFFLINE_INPUT_ROOT:-$DEFAULT_DISTRIBUTED_OFFLINE_INPUT_ROOT}
if [[ "$RUN_KIND" == "train" && -n "${TMPDIR_LUSTRE:-}" ]]; then
    DEFAULT_SHARED_RUNTIME_MODEL_ROOT="${TMPDIR_LUSTRE}/hierarchical_rema_train_${JOB_ID}/_runtime_active_models"
else
    DEFAULT_SHARED_RUNTIME_MODEL_ROOT="$DEFAULT_PERSIST_LOCAL_DIR/_runtime_active_models"
fi
SHARED_RUNTIME_MODEL_ROOT=${SHARED_RUNTIME_MODEL_ROOT:-$DEFAULT_SHARED_RUNTIME_MODEL_ROOT}
...
ALTERNATING_SELECTOR_NUM_DECOMPOSITIONS=${ALTERNATING_SELECTOR_NUM_DECOMPOSITIONS:-4}
ALTERNATING_SELECTOR_NUM_SELECTIONS=${ALTERNATING_SELECTOR_NUM_SELECTIONS:-4}
...
TRIVIAL_SINGLE_NODE_PENALTY=${TRIVIAL_SINGLE_NODE_PENALTY:-1.0}
TRIVIAL_SHALLOW_TWO_NODE_PENALTY=${TRIVIAL_SHALLOW_TWO_NODE_PENALTY:-0.5}
...
CONTROLLER_FALLBACK_POSITIVE_REWARD_SCALE=${CONTROLLER_FALLBACK_POSITIVE_REWARD_SCALE:-0.0}
...
MIN_WORKER_GRPO_GROUP_SIZE=${MIN_WORKER_GRPO_GROUP_SIZE:-3}
WORKER_LEARNING_RATE=${WORKER_LEARNING_RATE:-}
WORKER_EPOCHS=${WORKER_EPOCHS:-}
WORKER_CLIP_RANGE=${WORKER_CLIP_RANGE:-}
...
if [[ "$RUN_KIND" == "train" ]]; then
    DEFAULT_COPY_OUTPUTS_TO_PERSIST_LOCAL_DIR=false
    DEFAULT_STREAM_RUNTIME_LOG_TO_STDOUT=true
else
    DEFAULT_COPY_OUTPUTS_TO_PERSIST_LOCAL_DIR=true
    DEFAULT_STREAM_RUNTIME_LOG_TO_STDOUT=true
fi
COPY_OUTPUTS_TO_PERSIST_LOCAL_DIR=${COPY_OUTPUTS_TO_PERSIST_LOCAL_DIR:-$DEFAULT_COPY_OUTPUTS_TO_PERSIST_LOCAL_DIR}
STREAM_RUNTIME_LOG_TO_STDOUT=${STREAM_RUNTIME_LOG_TO_STDOUT:-$DEFAULT_STREAM_RUNTIME_LOG_TO_STDOUT}
...
mkdir -p "$RUN_ROOT" "$LOCAL_OUTPUT_DIR" "$SHARED_RUNTIME_MODEL_ROOT" "$DISTRIBUTED_OFFLINE_INPUT_ROOT"
if [[ "$COPY_OUTPUTS_TO_PERSIST_LOCAL_DIR" == "1" || "$COPY_OUTPUTS_TO_PERSIST_LOCAL_DIR" == "true" || "$COPY_OUTPUTS_TO_PERSIST_LOCAL_DIR" == "True" ]]; then
...
WORKER_LEARNING_RATE_FLAG=""
if [[ -n "$WORKER_LEARNING_RATE" ]]; then
    WORKER_LEARNING_RATE_FLAG="--worker-learning-rate ${WORKER_LEARNING_RATE}"
fi

WORKER_EPOCHS_FLAG=""
if [[ -n "$WORKER_EPOCHS" ]]; then
    WORKER_EPOCHS_FLAG="--worker-epochs ${WORKER_EPOCHS}"
fi

WORKER_CLIP_RANGE_FLAG=""
if [[ -n "$WORKER_CLIP_RANGE" ]]; then
    WORKER_CLIP_RANGE_FLAG="--worker-clip-range ${WORKER_CLIP_RANGE}"
fi
...
VLLM_SLEEP_MODE_FLAG="--disable-vllm-sleep-mode"
if [[ "$VLLM_ENABLE_SLEEP_MODE" == "1" || "$VLLM_ENABLE_SLEEP_MODE" == "true" || "$VLLM_ENABLE_SLEEP_MODE" == "True" ]]; then
...
if [[ -n "$GFAM_REWARD_MODEL_PKL_PATH" ]]; then
    PYTHON_BOOTSTRAP="mkdir -p ${TMPDIR}/gfam_sentence_transformers; export PYTHONPATH=${TMPDIR}/gfam_sentence_transformers:/verl/verl:\$PYTHONPATH; python3 -c \"import sentence_transformers, sklearn\" >/dev/null 2>&1 || python3 -m pip install --no-cache-dir --no-deps --target ${TMPDIR}/gfam_sentence_transformers sentence-transformers scikit-learn joblib threadpoolctl;"
fi
...
DEFAULT_PERSIST_LOCAL_DIR=$DEFAULT_PERSIST_LOCAL_DIR
DEFAULT_DISTRIBUTED_OFFLINE_INPUT_ROOT=$DEFAULT_DISTRIBUTED_OFFLINE_INPUT_ROOT
DEFAULT_SHARED_RUNTIME_MODEL_ROOT=$DEFAULT_SHARED_RUNTIME_MODEL_ROOT
DEFAULT_RAY_TMP_BASE=$DEFAULT_RAY_TMP_BASE
TMPDIR=${TMPDIR:-}
TMPDIR_LUSTRE=${TMPDIR_LUSTRE:-}
TMPDIR_LOCAL=${TMPDIR_LOCAL:-}
TMPDIR_SHM=${TMPDIR_SHM:-}
RUN_ROOT=$RUN_ROOT
LOCAL_OUTPUT_DIR=$LOCAL_OUTPUT_DIR
PERSIST_LOCAL_DIR=$PERSIST_LOCAL_DIR
DISTRIBUTED_OFFLINE_INPUT_ROOT=$DISTRIBUTED_OFFLINE_INPUT_ROOT
SHARED_RUNTIME_MODEL_ROOT=$SHARED_RUNTIME_MODEL_ROOT
RAY_LOCAL_TMPDIR=$RAY_LOCAL_TMPDIR
S3_OUTPUT_PATH=$S3_OUTPUT_PATH
COPY_OUTPUTS_TO_PERSIST_LOCAL_DIR=$COPY_OUTPUTS_TO_PERSIST_LOCAL_DIR
STREAM_RUNTIME_LOG_TO_STDOUT=$STREAM_RUNTIME_LOG_TO_STDOUT
WANDB_DIR=$WANDB_DIR
...
ALTERNATING_SELECTOR_NUM_DECOMPOSITIONS=$ALTERNATING_SELECTOR_NUM_DECOMPOSITIONS
ALTERNATING_SELECTOR_NUM_SELECTIONS=$ALTERNATING_SELECTOR_NUM_SELECTIONS
ALTERNATING_DECOMPOSER_NUM_DECOMPOSITIONS=$ALTERNATING_DECOMPOSER_NUM_DECOMPOSITIONS
ALTERNATING_DECOMPOSER_NUM_SELECTIONS=$ALTERNATING_DECOMPOSER_NUM_SELECTIONS
...
CONTROLLER_FALLBACK_POSITIVE_REWARD_SCALE=$CONTROLLER_FALLBACK_POSITIVE_REWARD_SCALE
TRIVIAL_SINGLE_NODE_PENALTY=$TRIVIAL_SINGLE_NODE_PENALTY
TRIVIAL_SHALLOW_TWO_NODE_PENALTY=$TRIVIAL_SHALLOW_TWO_NODE_PENALTY
...
MIN_WORKER_GRPO_GROUP_SIZE=$MIN_WORKER_GRPO_GROUP_SIZE
WORKER_LEARNING_RATE=${WORKER_LEARNING_RATE:-<auto>}
WORKER_EPOCHS=${WORKER_EPOCHS:-<auto>}
WORKER_CLIP_RANGE=${WORKER_CLIP_RANGE:-<auto>}
...
python3 -m hierarchical_rema.demo \
  ...
  --trivial-single-node-penalty ${TRIVIAL_SINGLE_NODE_PENALTY} \
  --trivial-shallow-two-node-penalty ${TRIVIAL_SHALLOW_TWO_NODE_PENALTY} \
  --temperature ${TEMPERATURE} \
...
python3 -m hierarchical_rema.train \
  ...
  ${PARAMETER_SHARING_FLAG} \
  --worker-base-model-path ${WORKER_BASE_MODEL_PATH} \
  --distributed-offline-input-root ${DISTRIBUTED_OFFLINE_INPUT_ROOT} \
  --shared-runtime-model-root ${SHARED_RUNTIME_MODEL_ROOT} \
  --num-decompositions ${NUM_DECOMPOSITIONS} \
  --num-selections ${NUM_SELECTIONS} \
  --alternating-selector-num-decompositions ${ALTERNATING_SELECTOR_NUM_DECOMPOSITIONS} \
  ...
  --controller-fallback-positive-reward-scale ${CONTROLLER_FALLBACK_POSITIVE_REWARD_SCALE} \
...
srun --overlap --nodes=1 --ntasks=1 ${RAY_DRIVER_NODE_FLAG} apptainer exec --nv --writable-tmpfs \
    ...
    "$LOCAL_SIF_IMAGE_PATH" \
    bash -c "$COMMAND" \
    > >(if [[ "$STREAM_RUNTIME_LOG_TO_STDOUT" == "1" || "$STREAM_RUNTIME_LOG_TO_STDOUT" == "true" || "$STREAM_RUNTIME_LOG_TO_STDOUT" == "True" ]]; then tee "$RUNTIME_LOG"; else cat > "$RUNTIME_LOG"; fi) \
    2>&1
RUN_EXIT_CODE=$?
set -e
